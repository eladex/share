	...
	function __identity(FileDirective) {
	    ...
	    return function (_FileDirective) {
	        ...
	        FileDrop.prototype.onDrop = function onDrop(event) {
                ...
                this._addAllFiles(transfer, this.uploader.addToQueue, this.uploader, [options, filters]);
//	            this.uploader.addToQueue(transfer.files, options, filters);
	         ...
	         * Helper [elad fix]
	         */
            FileDrop.prototype._addAllFiles = function _addAllFiles(dataTransfer, callback, 
            cb_this, cb_additional_args){
                
                var addFiles = function(item){
                    if(item.isFile){
                        item.file(function(file){
                            var additionalArgs = cb_additional_args.slice();
                            additionalArgs.unshift(file);
                            callback.apply(cb_this, additionalArgs); 
                        });
